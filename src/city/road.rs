...
//! actually look.
//!
//! Ticket 071 adds a second file per row — `<kind>-tunnel.nbt`, at the *same*
//! canonical orientation — for a cell the terrain closes over. That's a
//! [`RoadPieceVariant`], not a kind: it changes which `.nbt` a cell resolves
//! to, never which shape or rotation [`select_piece`] answers with.
//!
...
mod tests {
    ...
    fn connections_at_reports_exactly_the_road_neighbours() {
        ...
        city.add_road_cell(IVec2::new(0, 0), "dirt", 64, None, RoadPieceVariant::Surface).unwrap();
        city.add_road_cell(IVec2::new(0, -1), "dirt", 64, None, RoadPieceVariant::Surface).unwrap(); // north
        city.add_road_cell(IVec2::new(1, 0), "dirt", 64, None, RoadPieceVariant::Surface).unwrap(); // east
    ...
    fn connections_at_reports_a_full_cross() {
        ...
        for offset in [IVec2::new(0, -1), IVec2::new(0, 1), IVec2::new(1, 0), IVec2::new(-1, 0)] {
            city.add_road_cell(offset, "dirt", 64, None, RoadPieceVariant::Surface).unwrap();
        }
    ...
    fn reachable_from_a_single_cell_island_is_only_itself() {
        let mut city = City::default();
        city.add_road_cell(IVec2::new(5, 5), "dirt", 64, None, RoadPieceVariant::Surface).unwrap();
    ...
    fn reachable_from_a_loop_terminates_and_does_not_double_count() {
        ...
        for cell in [IVec2::new(0, 0), IVec2::new(1, 0), IVec2::new(0, 1), IVec2::new(1, 1)] {
            city.add_road_cell(cell, "dirt", 64, None, RoadPieceVariant::Surface).unwrap();
        }
    ...
    fn touching_road_cells_finds_every_cell_along_a_wide_footprint() {
        let mut city = City::default();
        city.add_road_cell(IVec2::new(0, 0), "dirt", 64, None, RoadPieceVariant::Surface).unwrap(); // block tiles 0..6 x 0..6
        city.add_road_cell(IVec2::new(1, 0), "dirt", 64, None, RoadPieceVariant::Surface).unwrap(); // block tiles 6..12 x 0..6
