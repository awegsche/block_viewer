...
use crate::chunk_pipeline::{
    poll_completed_chunk_loads, start_chunk_loads, InFlightChunkLoads, SpawnedChunkEntities,
};
...
/// re-requested, in the same frame.
...
impl Plugin for ChunkUnloadPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(
            Update,
            (
                cancel_out_of_range_in_flight_loads.before(poll_completed_chunk_loads),
                unload_chunks.before(start_chunk_loads),
            ),
        );
    }
}
...
/// Cancels (drops) any in-flight chunk-load task whose coordinate has left
/// render distance since it was kicked off. Simply dropping a `Task`
/// cancels it (`bevy_tasks::Task::cancel`'s doc comment: "it's possible to
/// simply drop the `Task` to cancel it"), so no `.await` is needed.
fn cancel_out_of_range_in_flight_loads(
